...
def main():
    ...
    warehouse = WarehouseEnv(warehouse_lvl, BLOCK_SIZE, NUM_AGENTS, gamma, batch_size, buffer_size,
        ...
        for step in itertools.count(): #Counts how many states has been visited
            ...
            for agent in warehouse.agents: #For each agent, perform actions
                if agent.done == False: #If agent done, stop making decisions
                    ...
                    rew, new_state, agent.done = warehouse.step(action, agent) #Take step, save reward, new state and if done
