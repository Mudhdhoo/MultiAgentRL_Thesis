...
class WarehouseEnv:
    ...
    def __init__(self, layout:list, block_size:int, num_agents:int, gamma, batch_size, buffer_size,
        ...
        self.level = []
        self.make_level()
        if level_name == "test_lvl34":
            self.starting_points = [120,80,40,80,80,40,80,120]
        elif level_name == "test_lvl34_2":
            self.starting_points = [40,120,120,120,40,40,120,40]
        elif level_name == "test_lvl34_3":
            self.starting_points = [60,160,140,160,100,160,180,160]
        ...
        for agent in self.agents:
            agent.communicate(self.agents,(self.WIDTH-40),(self.HEIGHT-40))      # Initialize communication between agents
    ...
    def step(self) -> tuple:
        ...
        for agent in self.agents:
            ...
            if agent.done == False:
                for wall in self.level:
                    ...
                    if pygame.Rect.colliderect(agent.rect, wall.wall_element) and wall.texture == self.PICKUP_TEXTURE and not agent.has_package and wall.index == ((agent.index+1)%4+1): #Replace 4 with self.NUM_AGENTS
        ...
        for agent in self.agents:
            agent.communicate(self.agents,(self.WIDTH-40),(self.HEIGHT-40))      # Get state information from other agents
        return
            #return reward, agent.state, done
    ...
    def reset(self) -> None:
        ...
        for agent in self.agents:
            agent.communicate(self.agents,(self.WIDTH-40),(self.HEIGHT-40))
    ...
    def generate_starting_points(self) -> None:
        ...
        while collide:
            ...
            if self.agents_created == True:
                for agent in self.agents:
                    if x_pos == agent.position[0] and y_pos == agent.position[1]:
                        collide = True
                        break
