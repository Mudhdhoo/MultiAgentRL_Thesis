...
#
    ...
    def plot(self):
            ...
            plt.savefig(F'{self.level_name}-Functional/{self.num_agents}Agent/AvgReward_{self.level_name}_{self.eps_dec}Steps_{self.update_freq}_Batchsize{self.batch_size}_lr{self.lr}_Num_Agents_{self.num_agents}.png')
            ...
            plt.savefig(F'{self.level_name}-Functional/{self.num_agents}Agent/Loss_{self.level_name}_{self.eps_dec}Steps_{self.update_freq}_Batchsize{self.batch_size}_lr{self.lr}_Num_Agents_{self.num_agents}.png')

            with open(F"{self.level_name}-Functional/{self.num_agents}Agent/Level_{self.level_name}_Steps_{self.eps_dec}_Buffersize_{self.buffer_size}_Batchsize_{self.batch_size}_Lr_{self.lr}_Num_Agents_{self.num_agents}_Trained_Network_Agent{self.index}.csv", 'w', newline='') as file:
    ...
    def save_model(self):
        #Saves the model
        torch.save(self.net_online.state_dict(), F"{self.level_name}-Functional/{self.num_agents}Agent/Level_{self.level_name}_Steps_{self.eps_dec}_Buffersize_{self.buffer_size}_Batchsize_{self.batch_size}_Lr_{self.lr}_Num_Agents_{self.num_agents}_Trained_Network_Agent{self.index}")

    def load_model(self):
        #Loads trained models
        model = NN(((self.num_agents*2)+1))
        model.load_state_dict(torch.load(F"{self.level_name}-Functional/{self.num_agents}Agent/Level_{self.level_name}_Steps_{self.eps_dec}_Buffersize_{self.buffer_size}_Batchsize_{self.batch_size}_Lr_{self.lr}_Num_Agents_{self.num_agents}_Trained_Network_Agent{self.index}",map_location=self.device))
    ...
    def communicate(self, other_agents:list,width,height):
        """
        Updates the agent state by gathering information about other agents coordinates. Updates pickup status
        """
        self.state = np.array([[float(agent.position[0]/width),float(agent.position[1]/height)] for agent in other_agents]).flatten()     # Get every other agents position
