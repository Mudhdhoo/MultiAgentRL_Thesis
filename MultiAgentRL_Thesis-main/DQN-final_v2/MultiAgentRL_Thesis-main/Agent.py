...
class Agent:
    ...
    def __init__(self, init_pos:tuple, size:int, texture:pygame.image, num_agents, gamma, batch_size, buffer_size,
        ...
        self.optimizer = optim.Adam(self.net_online.parameters(), lr=1e-3)
    ...
    def grad_step(self,step):
        ...
        actions_t = torch.as_tensor(actions, device=self.device, dtype=torch.int64)  #Convert to torch-tensor
    ...
    def plot_reward(self):
            ...
            plt.savefig(F'AvgReward_{self.level_name}_{self.eps_dec}Steps_{self.update_freq}_Batchsize{self.batch_size}.png')

    def save_model(self):
        #Saves the model
        torch.save(self.net_online.state_dict(), F"Level_{self.level_name}_Steps_{self.eps_dec}_Buffersize_{self.buffer_size}_Batchsize_{self.batch_size}_Num_Agents_{self.num_agents}_Trained_Network_Agent{self.index}")

    def load_model(self):
        #Loads trained models
        model = NN(((self.num_agents*2)+1))
        model.load_state_dict(torch.load(F"Level_{self.level_name}_Steps_{self.eps_dec}_Buffersize_{self.buffer_size}_Batchsize_{self.batch_size}_Num_Agents_{self.num_agents}_Trained_Network_Agent{self.index}",map_location=self.device))
