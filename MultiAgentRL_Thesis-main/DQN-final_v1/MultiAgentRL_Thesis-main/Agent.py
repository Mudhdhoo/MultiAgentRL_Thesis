...
class Agent:
    ...
    def grad_step(self,step):
        ...
        targets = rewards_t + self.gamma*max_target_q_values #(1-dones_t)*Calculate target
    ...
    def save_model(self):
        #Saves the model
        torch.save(self.net_online.state_dict(), F"Level_{self.level_name}_Num_Agents_{self.num_agents}_Trained_Network_Agent{self.index}")

    def load_model(self):
        #Loads trained models
        model = NN(((self.num_agents*2)+1))
        model.load_state_dict(torch.load(F"Level_{self.level_name}_Num_Agents_{self.num_agents}_Trained_Network_Agent{self.index}"))
